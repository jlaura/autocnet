...
from autocnet.io.db.model import Points, Measures, Images, GroundPoint
...
def propagate_ground_point(point,
    ...
    for _, image in images.iterrows():
        ...
        if image['id'] == best_results[0][8]:
            # Measure already added as the best
            point.measures[0].aprioriline = line
            point.measures[0].apriorisample = sample
...
def find_most_interesting_ground(apriori_lon_lat, ground_mosaic, cam_type='isis',size=71, ncg=None, Session=None):
    """
    ...
    with ncg.session_scope() as session:
        g = GroundPoint(path=ground_mosaic.file_name,
